...
public final class Buffer extends Table {
  ...
  public BufferT unpack() {
    BufferT _o = new BufferT();
    unpackTo(_o);
    return _o;
  }
  public void unpackTo(BufferT _o) {
    me.hydos.trifecta.flatbuffers.Titan.Model.IndexesT[] _oIndexBuffer = new me.hydos.trifecta.flatbuffers.Titan.Model.IndexesT[indexBufferLength()];
    for (int _j = 0; _j < indexBufferLength(); ++_j) {_oIndexBuffer[_j] = (indexBuffer(_j) != null ? indexBuffer(_j).unpack() : null);}
    _o.setIndexBuffer(_oIndexBuffer);
    me.hydos.trifecta.flatbuffers.Titan.Model.VerticesT[] _oVertexBuffer = new me.hydos.trifecta.flatbuffers.Titan.Model.VerticesT[vertexBufferLength()];
    for (int _j = 0; _j < vertexBufferLength(); ++_j) {_oVertexBuffer[_j] = (vertexBuffer(_j) != null ? vertexBuffer(_j).unpack() : null);}
    _o.setVertexBuffer(_oVertexBuffer);
    me.hydos.trifecta.flatbuffers.Titan.Model.MorphsT[] _oMorphs = new me.hydos.trifecta.flatbuffers.Titan.Model.MorphsT[morphsLength()];
    for (int _j = 0; _j < morphsLength(); ++_j) {_oMorphs[_j] = (morphs(_j) != null ? morphs(_j).unpack() : null);}
    _o.setMorphs(_oMorphs);
  }
  public static int pack(FlatBufferBuilder builder, BufferT _o) {
    if (_o == null) return 0;
    int _indexBuffer = 0;
    if (_o.getIndexBuffer() != null) {
      int[] __indexBuffer = new int[_o.getIndexBuffer().length];
      int _j = 0;
      for (me.hydos.trifecta.flatbuffers.Titan.Model.IndexesT _e : _o.getIndexBuffer()) { __indexBuffer[_j] = me.hydos.trifecta.flatbuffers.Titan.Model.Indexes.pack(builder, _e); _j++;}
      _indexBuffer = createIndexBufferVector(builder, __indexBuffer);
    }
    int _vertexBuffer = 0;
    if (_o.getVertexBuffer() != null) {
      int[] __vertexBuffer = new int[_o.getVertexBuffer().length];
      int _j = 0;
      for (me.hydos.trifecta.flatbuffers.Titan.Model.VerticesT _e : _o.getVertexBuffer()) { __vertexBuffer[_j] = me.hydos.trifecta.flatbuffers.Titan.Model.Vertices.pack(builder, _e); _j++;}
      _vertexBuffer = createVertexBufferVector(builder, __vertexBuffer);
    }
    int _morphs = 0;
    if (_o.getMorphs() != null) {
      int[] __morphs = new int[_o.getMorphs().length];
      int _j = 0;
      for (me.hydos.trifecta.flatbuffers.Titan.Model.MorphsT _e : _o.getMorphs()) { __morphs[_j] = me.hydos.trifecta.flatbuffers.Titan.Model.Morphs.pack(builder, _e); _j++;}
      _morphs = createMorphsVector(builder, __morphs);
    }
    return createBuffer(
      builder,
      _indexBuffer,
