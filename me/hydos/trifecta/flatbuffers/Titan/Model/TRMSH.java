...
public final class TRMSH extends Table {
  ...
  public TRMSHT unpack() {
    TRMSHT _o = new TRMSHT();
    unpackTo(_o);
    return _o;
  }
  public void unpackTo(TRMSHT _o) {
    long _oUnk0 = unk0();
    _o.setUnk0(_oUnk0);
    me.hydos.trifecta.flatbuffers.Titan.Model.MeshShapeT[] _oMeshes = new me.hydos.trifecta.flatbuffers.Titan.Model.MeshShapeT[meshesLength()];
    for (int _j = 0; _j < meshesLength(); ++_j) {_oMeshes[_j] = (meshes(_j) != null ? meshes(_j).unpack() : null);}
    _o.setMeshes(_oMeshes);
    String _oBufferName = bufferName();
    _o.setBufferName(_oBufferName);
  }
  public static int pack(FlatBufferBuilder builder, TRMSHT _o) {
    if (_o == null) return 0;
    int _meshes = 0;
    if (_o.getMeshes() != null) {
      int[] __meshes = new int[_o.getMeshes().length];
      int _j = 0;
      for (me.hydos.trifecta.flatbuffers.Titan.Model.MeshShapeT _e : _o.getMeshes()) { __meshes[_j] = me.hydos.trifecta.flatbuffers.Titan.Model.MeshShape.pack(builder, _e); _j++;}
      _meshes = createMeshesVector(builder, __meshes);
    }
    int _bufferName = _o.getBufferName() == null ? 0 : builder.createString(_o.getBufferName());
    return createTRMSH(
      builder,
      _o.getUnk0(),
      _meshes,
