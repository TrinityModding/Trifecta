...
public final class Lod extends Table {
  ...
  public void unpackTo(LodT _o) {
    me.hydos.trifecta.flatbuffers.Titan.Model.LodIndexT[] _oIndex = new me.hydos.trifecta.flatbuffers.Titan.Model.LodIndexT[indexLength()];
    for (int _j = 0; _j < indexLength(); ++_j) {_oIndex[_j] = (index(_j) != null ? index(_j).unpack() : null);}
    _o.setIndex(_oIndex);
    String _oLodType = lodType();
    _o.setLodType(_oLodType);
  }
  public static int pack(FlatBufferBuilder builder, LodT _o) {
    if (_o == null) return 0;
    int _index = 0;
    if (_o.getIndex() != null) {
      int[] __index = new int[_o.getIndex().length];
      int _j = 0;
      for (me.hydos.trifecta.flatbuffers.Titan.Model.LodIndexT _e : _o.getIndex()) { __index[_j] = me.hydos.trifecta.flatbuffers.Titan.Model.LodIndex.pack(builder, _e); _j++;}
      _index = createIndexVector(builder, __index);
    }
    int _lodType = _o.getLodType() == null ? 0 : builder.createString(_o.getLodType());
    return createLod(
      builder,
      _index,
      _lodType);
